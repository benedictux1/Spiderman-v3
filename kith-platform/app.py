...
def get_graph_data():
    ...
    try:
        ...
        # Create nodes - add "You" as the central node
        nodes = [
            {
                "id": 0,
                "label": "You",
        ...
        for contact in contacts:
            nodes.append({
                "id": contact.id,
                "label": contact.full_name,
                "color": "#4ecdc4" if contact.tier == 1 else "#45b7d1",
                "size": 25 if contact.tier == 1 else 20,
                "group": f"tier{contact.tier}"
            })
        
        # Get relationships
        relationships = session.query(ContactRelationship).all()
        edges = []
        ...
        # If no relationships exist, connect all contacts to "You"
        if not edges:
            for contact in contacts:
                edges.append({
                    "from": 0,  # You
                    "to": contact.id,
                    "label": f"Tier {contact.tier}"
                })
        ...
        # Return empty graph data instead of error to prevent UI issues
        return jsonify({
            "nodes": [{"id": 0, "label": "You", "color": "#ff6b6b", "size": 30}],
            "edges": [],
            "groups": {"user": {"color": "#ff6b6b"}}
        })
    finally:
        session.close()
