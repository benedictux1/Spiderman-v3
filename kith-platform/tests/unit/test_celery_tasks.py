...
class TestAITasks:
    ...
    def test_process_note_async_success(self, mock_note_service_class):
        ...
        # Mock the update_state method to prevent Celery backend calls
        with patch.object(process_note_async, 'update_state', Mock()):
            result = process_note_async.run(
                1,  # contact_id
    ...
    @patch('app.tasks.ai_tasks.NoteService')
    def test_process_note_async_failure(self, mock_note_service_class):
        ...
        with patch.object(process_note_async, 'update_state', Mock()):
            ...
            with pytest.raises(Exception, match="Processing failed"):
                process_note_async.run(1, "test", 1)
    ...
    def test_batch_process_notes_success(self, mock_process_note_async):
        ...
        # Mock the update_state method to prevent Celery backend calls
        with patch.object(batch_process_notes, 'update_state', Mock()):
            result = batch_process_notes.run(note_data)
    ...
    def test_batch_process_notes_failure(self, mock_process_note_async):
        ...
        with patch.object(batch_process_notes, 'update_state', Mock()):
            ...
            with pytest.raises(Exception, match="Batch failed"):
                batch_process_notes.run(note_data)
...
class TestTelegramTasks:
    ...
    def test_sync_telegram_contacts_success(self, mock_telegram_service_class):
        ...
        # Mock the update_state method to prevent Celery backend calls
        with patch.object(sync_telegram_contacts, 'update_state', Mock()):
            result = sync_telegram_contacts.run(user_id=1)
    ...
    def test_process_telegram_messages_success(self):
        ...
        # Mock the update_state method to prevent Celery backend calls
        with patch.object(process_telegram_messages, 'update_state', Mock()):
            result = process_telegram_messages.run(user_id=1, message_data=message_data)
    ...
    def test_process_telegram_messages_failure(self):
        ...
        with patch.object(process_telegram_messages, 'update_state', Mock()):
            ...
            with patch('app.tasks.telegram_tasks.logger') as mock_logger:
                # This would need to be implemented to actually fail
                result = process_telegram_messages.run(user_id=1, message_data=[])
                assert result['user_id'] == 1
